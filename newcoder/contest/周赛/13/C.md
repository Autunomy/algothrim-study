先预处理每种字符的数量，再一次遍历str的每个字符，判断能替换的字符数量，并累加到答案中即可
...
public class Main {
    ...
    void solve(){
        ...
        for(int i=0;i<str.length();++i){
            char c = str.charAt(i);
            if(c >= 'a' && c <= 'z'){
                if(nums[0] - 1 > 0) res += (25 + 26 + 10 + 4);
                else res += 25;
            }else if(c >= 'A' && c <= 'Z'){
                if(nums[1] - 1 > 0) res += (25 + 26 + 10 + 4);
                else res += 25;
            }else if(c >= '0' && c <= '9'){
                if(nums[2] - 1 > 0) res += (26 + 26 + 9 + 4);
                else res += 9;
            }else{
                if(nums[3] - 1 > 0) res += (26 + 26 + 10 + 3);
                else res += 3;
            }
        }
