题意：给出一个数组，编号为1~n可以进行两种操作：
1. 选一个奇数下标，**将元素加入到答案中**并删除这个元素，此时后面元素向前移动一位
2. 选一个偶数下标，删除这个元素，此时后面元素向前移动一位

求可以得到的最大分数

思路：
我们只需要判断前两个元素的正负，后面的元素都可以利用前面元素的删除来实现奇偶性的改变。以下是几种情况
1. a[1] > 0 a[2] > 0  每次选择1号位置即可
2. a[1] > 0 a[2] < 0  可以通过选择a[1]来改变后面元素的奇偶性
3. a[1] < 0 a[2] < 0  可以通过选择a[2]来改变后面元素的奇偶性
4. a[1] < 0 a[2] > 0  求全部正数的和最后减去a[2]或加上a[1]并求两者大的
...
public class Main {
    ...
    void solve(){
        ...
        if(n >= 2 && arr[0] < 0 && arr[1] > 0){
            sum = Math.max(sum + arr[0],sum - arr[1]);
        }
