首先判断a中有没有小于k的数，如果有，那么res的最小值为1，代表最小子数组长度为1，
接下来遍历h，找到所有hi都能被hi+1整除的子数组，对于每个子数组：
    判断以每个元素为右边界的最长子数组长度是多少然后更新一下答案
...
public class Main {
    ...
    void solve(){
        ...
        while(idx < n){
            if(h[idx - 1] >= h[idx] && h[idx - 1] % h[idx] == 0){
                int left = idx - 1 ,right = idx;
                int sum = a[idx - 1];
                //在判断的过程中每次都以right为右边界，判断当前合法的最长子数组的长度
                while(right < n && h[right - 1] >= h[right] && h[right - 1] % h[right] == 0) {
                    sum += a[right];
                    if(sum <= k){//如果<=k 则满足条件，直接更新答案即可
                        res = Math.max(res,right - left + 1);
                    }else{
                        while(sum > k){//因为是以当前元素为右边界，所以让left不断增大即可
                            sum -= a[left ++];
                        }
                        res = Math.max(res,right - left + 1);
                    }
