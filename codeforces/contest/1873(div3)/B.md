排序后给数组第一个元素+1，然后将全部元素乘起来即可。注意要使用long，防止爆int
为什么是给数组第一个元素+1而不是最后一个元素+1？
因为给第一个元素+1的意思 (arr[0] + 1) * arr[1] * arr[2] ... arr[n - 1] => arr[0] * arr[1] * arr[2] ... + arr[1] * arr[2] ... arr[n - 1]
但是给最后一个元素+1     arr[0] * arr[1] * ... (arr[n - 1] + 1) => arr[0] * arr[1] * ... arr[n - 1] + arr[0] * arr[1] * ... arr[n - 2]
我们能够明显看出来 arr[1] * arr[2] ... arr[n - 1] > arr[0] * arr[1] * ... arr[n - 2]
...
public class Main {
    ...
    void solve(){
        ...
        arr[0] ++;
        long res = 1;
        for(int i=0;i<n;++i) res *= (long)arr[i];
