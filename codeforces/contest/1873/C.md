遍历，然后判断每个X所处的位置即可。x方向上的分数就是与左边界的距离和右边界的距离的最小值，y方向同理，然后当前点对应的分值就是min(x,y)，然后累加到答案中即可
...
public class Main {
    ...
    void solve(){
        ...
        for(int i=1;i<=10;++i){
            String str = sc.next();
            str = "0" + str;//因为下标从1开始，给最前面补一个字符，方便使用
            for(int j=1;j<=10;++j){
                char c = str.charAt(j);

                if(c == 'X'){
                    int x = Math.min(i,11-i);
                    int y = Math.min(j,11-j);
                    res += Math.min(x,y);
                }
            }
        }
