1~n中k个数字能成的最大值是[n-k+1,n]的和，最小值是[1,k]的和，只要x在这之间就一定能被表示，因为我们总能够通过移动某些数字实现+1和-1来形成x
...
public class Main {
    ...
    void solve(){
        ...
        long sumpre = (long)k * (k + 1) / 2;
        long sumsuf = (long)k * (n - k + 1) + (long)k * (k - 1) / 2;

        if(x >= sumpre && x <= sumsuf){
