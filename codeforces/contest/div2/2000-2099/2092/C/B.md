所有a字符串奇数位置的字符可以与任意b字符串偶数位置的字符交换(可以通过题目中给出的交换方式执行若干次)
所以只要判断a的奇数位置的1个数少于b偶数位置0，同时a偶数位置1数量少于b奇数位置0的数量即可。
...
public class Main {
    ...
    void solve(){
        ...
        int jiNum = 0;
        for(int i = 0; i < n; ++ i) {
            if(i % 2 == 0 && b.charAt(i) == '0') ouNum ++;
            else if(i % 2 == 1 && b.charAt(i) == '0') jiNum ++;
        }
        ...
        if(ouNum < 0 || jiNum < 0) System.out.println("NO");
