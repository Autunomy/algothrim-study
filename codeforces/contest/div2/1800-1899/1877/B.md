思路：将a和b放在同一个数组c中，这样方便进行排序，排序按照b的大小进行排序，从小到大。最开始的时候先告知一个人，然后以这一个人为起点不断的扩散
结束条件就是所有人被通知完或者当前这个人通知需要的价值超过了p就结束，最后将没有通知的人都作为最开始需要被通知的人也就是加上剩余人数乘以p。
为什么大于p的人都需要作为最开始通知的一批人？
因为这批人不能去给其他人通知，因为所需的代价更大，只需要最开始对他进行通知即可，通知代价小于p的人才有利用价值
...
public class Main {
    ...
    void solve(){
        ...
        //使用一个最小的b开始，所以需要初始值为p
        long res = p;
        //已经传播了一个需要以1开始
        int num = 1;

        for(int i=0;i<n && c[i][1] <= p;++i){
            int temp = Math.min(n-num,c[i][0]);
            res += (long)temp * c[i][1];
            num += temp;
        }
