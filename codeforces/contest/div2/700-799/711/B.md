...
题意:
给出一个矩阵，其中的一个数字是0代表需要给这个位置填入一个适当的数，使矩阵满足每一行的和，每一列的和，整个矩形的对角线的和都相等，填入的数字必须大于等于1

思路:
随便找出一行的和，然后让存在0的行与这一行相等，填入一个x，判断填入之后是否能保证条件，如果不能，就返-1
