...
有一个地面大小为4 * 4
要求将给出的s.length()个砖块放入地面中，如果在某个时刻，当前这一行地面铺满了瓷砖，则将这一行的瓷砖都移除，详情可看样例，求每个砖块可以防止的位置。

思路：
每四个垂直防止的砖块就可以将两行地面的瓷砖清除，每两个水平防止的瓷砖就可以将一行地面清除，只需要将所有的垂直砖块放在前两行中，从1摆到4，然后再从1摆到4，依次循环。所有水平砖块放在第三行，两个为一组。
