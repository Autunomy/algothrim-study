题意：
你得到一个长度为 n的数组 a您可以对其执行以下操作：
1. 给[0,i]的子数组都加上x
2. 给[0,i]的子数组都modx

判断是否能将给定的数组变为一个严格递增的数组？

思路：从后向前遍历，将所有的arr[i]都变为i+1。过程中需要记录后面的元素给前缀增加的数，方便前面的数取模和加上x
...
public class Main {
    ...
    void solve(){
        ...
        for (int i=n-1;i>=0;i--)
        {
            int add=(i-(arr[i]+sum)%n+n)%n;
            System.out.println("1 "+(i+1) + " "+add);
            sum+=add;
        }
