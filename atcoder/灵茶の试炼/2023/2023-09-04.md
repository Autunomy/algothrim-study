[原题链接](https://atcoder.jp/contests/diverta2019/tasks/diverta2019_c)

首先是统计字符串中的AB数量
然后统计三种字符串的数量:A结尾 B开头A结尾 B开头
res += min(a,b) + ba  因为每个ba都可以插入到a结尾和b开头的两个字符串中间
注意一种特殊情况：a和b为0且ba>0 这种情况情况下res += ba - 1
    ...
    void solve(){
        ...
        for(int i=0;i<n;++i) {
            ...
            for(int j=0;j<s.length();++j){
                if(s.charAt(j) == 'A' && j + 1 < s.length() && s.charAt(j + 1) == 'B'){
                    res ++;
                }
            }
        }
        ...
        if(arr[0] == 0 && arr[2] == 0) {
            if(arr[1] > 0) res += arr[1] - 1;
        }
        else{
            res += Math.min(arr[0],arr[2]) + arr[1];
        }
