思路：求个前缀和则 k = sum[R] - sum[L] 也就是求 sum[R] - k = sum[L]的数量。使用一个hash表保存所有sum[L]的数量，然后将答案进行累加即可
注意一个地方就是sum[L]还需要初始化一个{0:1} 因为sum[L]所包含元素数量可能为0，这也算一种

题目连接:[https://atcoder.jp/contests/abc233/submit?taskScreenName=abc233_d](https://atcoder.jp/contests/abc233/submit?taskScreenName=abc233_d)
...
public class Main {
    ...
    void solve(){
        ...
        map.put(0L,1L);//表示从开始选到R结束，也就是说sum[L] = 0

        long res = 0;
        for(int i=1;i<=n;++i){
            sum += sc.nextLong();
            if(map.get(sum - k) != null) res += map.get(sum - k);
