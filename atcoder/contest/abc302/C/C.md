题意：
给定n个字符串，可以对所有的字符串中的字母重新排列，所有字符串也可以重新排列顺序，判断是否可以新形成一个顺序，让ti字符串与ti+1字符串只相差一个字母
...
public class Main {
    ...
    void solve(){
        ...
        if(dfs(cnt,new HashSet<>(),-1)) System.out.println("Yes");
        else System.out.println("No");
    }

    boolean dfs(int[][] cnt,Set<Integer> set,int last){
        if(set.size() == cnt.length) return true;
        boolean res = false;
        for(int i=0;i<cnt.length;++i){
            if(last == -1){
                set.add(i);
                res = res || dfs(cnt,set,i);
                set.remove(i);
            }
            else if(!set.contains(i)){
                int num = 0;
                for(int k=0;k<26;++k){
                    num += Math.abs(cnt[i][k] - cnt[last][k]);
                }
                if(num/2 == 1){
                    set.add(i);
                    res = res || dfs(cnt,set,i);
