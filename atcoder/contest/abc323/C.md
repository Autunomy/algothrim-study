...
public class Main {
    ...
    void solve(){
        ...
        for(int i=0;i<n;++i){
            int cnt = 0;
            for(int j=0;j<m && scores[i] < max;++j){
                if(str[i].charAt(temp[j][1]) == 'x'){
                    scores[i] += temp[j][0];
                    cnt ++;
                }
            }
    ...
    void reverseArr(int[] arr){int i=0,j=arr.length-1;while(i < j){swap(arr,i,j);i++;j--;}}
    void reverseArr(long[] arr){int i=0,j=arr.length-1;while(i < j){swap(arr,i,j);i++;j--;}}
    ...
    void swap(long[] arr,int i,int j){long a = arr[i];arr[i] = arr[j];arr[j] = a;}
    //棋盘距离
