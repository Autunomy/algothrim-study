    ...
    void solve(){
        ...
        for(int i=0;i<n-1;++i){
            for(int j=0;j<=n-i-2;++j){
                arr[i][j+i+1] = sc.nextInt();
                arr[j+i+1][i] = arr[i][j+i+1];
            }
        }

        int[] nums = new int[n];
        // dfs(nums,new ArrayList<>());
        System.out.println(dfs(nums,new ArrayList<>()));
    }

    long dfs(int[] nums,List<Integer> list){
        if(map.get(getString(list)) != null){
            return map.get(getString(list));
        }
        long res = 0;
        for(int i=0;i<n;++i){
            ...
            for(int j=0;j<n;++j){
                ...
                res = Math.max(res,dfs(nums,list) + arr[i][j]);
                // long temp = map.get(getString(list)) == null ? 0 : map.get(getString(list));
                // map.put(getString(list),Math.max(res,temp));
        ...
        map.put(getString(list),res);
        return res;
    }

    String getString(List<Integer> list){
        List<Integer> bk = new ArrayList<>(list);
        Collections.sort(bk);
