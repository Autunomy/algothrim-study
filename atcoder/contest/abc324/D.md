求题目给出的数字的所有组合数中，是完全平方数的数量

由于长度最多为13所以就可以枚举所有的数字并求其完全平方数，判断其中的数字组成是否与题目给定的数字相同即可

为什么不会超时？
因为最多是9999999999999 它的根是3162277.6602 也就是10^7次方级别然后还会在进行一次比对所以最终的复杂度是10^8次方级别，可以通过
...
public class Main {
    ...
    void solve(){
        ...
        int[] arr = new int[10];
        for(int i=0;i<n;++i) arr[str.charAt(i)-'0'] ++;
        ...
        // System.out.println((int)Math.sqrt(max) + 1);
