判断哪些s可以通过增删改(最多一次)变成t

双指针同时遍历s和t判断操作次数，如果超过1次就不行
...
public class Main {
    ...
    void solve(){
        ...
        for(int i=0;i<n;++i){
            ...
            while(a < t1.length() && b < str.length()){
                if(t1.charAt(a) != str.charAt(b)){
                    if(t1.length() > str.length()){
                        a++;
                    }else if(t1.length() < str.length()){
                        b++;
                    }else{
                        a ++;
                        b ++;
            ...
            cnt += Math.max(t1.length() - a,str.length() - b);
            if(cnt <= 1){
                list.add(i + 1);
            }
        }
