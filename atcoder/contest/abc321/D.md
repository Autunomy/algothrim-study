思路：对b进行排序并求前缀和，遍历a的过程中二分找小于a[i] + b[i] <= p的最大的b[i]然后就可以得到a可以组成的最大价值并累加到答案中
...
public class Main {
    ...
    void solve(){
        ...
        for(int i=0;i<n;++i){
            ...
            int idx = lowerBound(b,0,b.length-1,p-a[i]);

            res += sum[idx + 1] + (idx + 1) * (long)a[i] + (long)p * (m - idx - 1);
